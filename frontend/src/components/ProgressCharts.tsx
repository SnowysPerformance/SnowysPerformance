...
// Charts, PRs, and wearable recovery for one athlete — shared by the coach's
// per-athlete Progress tab (showFatigue=true, since overtraining risk is a
// coaching tool) and the athlete's own Progress page (showFatigue=false —
// athletes see their charts and recovery, not a risk flag on themselves).
export default function ProgressCharts({ athleteId, showFatigue }: { athleteId: string; showFatigue: boolean }) {
  ...
  useEffect(() => {
    ...
    setFatigue(null);
    (async () => {
      const calls: Promise<any>[] = [api(`/api/workouts?athleteId=${athleteId}`)];
      if (showFatigue) calls.push(api(`/api/fatigue/${athleteId}`).catch(() => null));
      const [workoutLogs, fatigueData] = await Promise.all(calls);
      setLogs(workoutLogs);
      if (showFatigue) setFatigue(fatigueData);
    })();
  }, [athleteId, showFatigue]);
  ...
  return (
    <div className="space-y-6">
      <WearablePanel athleteId={athleteId} context={showFatigue ? "coach" : "self"} />
      ...
      {showFatigue && fatigue && (
        <div className="bg-surface border border-edge rounded p-4">
          ...
          <div className="text-sm text-faint mt-2">
            ACWR: {fatigue.acwr ?? "—"} · Acute load: {fatigue.acuteLoad} · Chronic (wk avg): {fatigue.chronicLoad}
          </div>
