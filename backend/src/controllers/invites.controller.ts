...
// This is the only way an athlete account can now be created on a team —
// there's no more open self-registration with just a Team ID.
export async function createInvite(req: Request, res: Response) {
  if (req.user!.role !== "COACH") return res.status(403).json({ error: "Only coaches can invite athletes" });
  ...
  const invite = await prisma.invite.create({
    data: { teamId: req.user!.teamId, email, token, invitedById: req.user!.userId, expiresAt },
  });
...
export async function getInviteByToken(req: Request, res: Response) {
  ...
  if (!invite || invite.usedAt || invite.expiresAt < new Date()) {
    return res.status(404).json({ error: "This invite link is invalid or has expired. Ask your coach to send a new one." });
  }
  res.json({ email: invite.email, teamName: invite.team.name });
}
...
export async function acceptInvite(req: Request, res: Response) {
  ...
  if (!invite || invite.usedAt || invite.expiresAt < new Date()) {
    return res.status(400).json({ error: "This invite link is invalid or has expired. Ask your coach to send a new one." });
  }
  ...
  const user = await prisma.user.create({
    data: { email: invite.email, passwordHash, name, role: "ATHLETE", teamId: invite.teamId },
  });
  await prisma.invite.update({ where: { id: invite.id }, data: { usedAt: new Date() } });

  const jwtToken = signToken({ userId: user.id, role: "ATHLETE", teamId: invite.teamId });
